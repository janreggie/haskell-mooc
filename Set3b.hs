...
sums :: Int -> [Int]
sums i = sumsOf [1..i] -- technically this *is* correct, and it *will* work.
...
indexDefault (x:_xs) 0 _def = x
indexDefault (_x:xs) i def = indexDefault xs (i-1) def
...
merge :: [Int] -> [Int] -> [Int]
merge xs [] = xs
merge xs (y:ys) = merge (mergeInto xs y) ys
  where
    mergeInto [] y = [y]
    mergeInto (x:xs) y
      | x <= y = x : mergeInto xs y
      | otherwise = y : x : xs
