...
countdown n = countdownHelper n "Ready! "
  where
    ellipsize n = show n ++ "... "
    countdownHelper 0 str = str ++ "Liftoff!"
    countdownHelper i str = countdownHelper (i -1) (str ++ ellipsize i)
...
smallestDivisor n = findDivisor n 2
  where
    findDivisor n i = if mod n i == 0 then i else findDivisor n (i + 1)
