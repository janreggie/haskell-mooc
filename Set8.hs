...
blendColor (Color r1 b1 g1) (Color r2 b2 g2) = Color (ave r1 r2) (ave b1 b2) (ave g1 g2)
  where
    ave x y = div (x+y) 2
...
paintSolid color (Shape s) (Picture p) = Picture f
  where
    f c = if s c then color else p c
------------------------------------------------------------------------------
...
paint (Picture pat) (Shape s) (Picture base) = Picture f
  where
    f c = if s c then pat c else base c
------------------------------------------------------------------------------
...
instance Transform Fill where
  apply (Fill clr) _ = Picture (const clr)
...
instance Transform Blur where
  apply Blur (Picture p) = Picture f
    where
      f (Coord x y) = blendMultiple (map (\(cx, cy) -> p $ Coord cx cy) [(x-1,y), (x,y), (x+1,y), (x,y-1), (x,y+1)])
      blendMultiple :: [Color] -> Color
      blendMultiple xs = Color (aveRed xs) (aveGreen xs) (aveBlue xs)
      aveRed xs = div (sum $ map getRed xs) (length xs)  
      aveGreen xs = div (sum $ map getGreen xs) (length xs)  
      aveBlue xs = div (sum $ map getBlue xs) (length xs)  
------------------------------------------------------------------------------
