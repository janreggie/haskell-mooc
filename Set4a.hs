...
middle x y z
  | compare x y == compare y z = y
  | compare y z == compare z x = z
  | otherwise = x
...
incrementKey k kvs = map (\kv -> if fst kv == k then (fst kv, snd kv + 1) else kv) kvs
...
winner scores player1 player2 = if score2 > score1 then player2 else player1
  where
